...
def download_data(period_weeks, period_hours):
    ...
    usd_krw_data = yf.download('USDKRW=X', start=start, end=end, interval='1h')  # USD/KRW 환율 데이터 다운로드 (1시간 간격)
    ...
    last_24_hours_data = usd_krw_data[usd_krw_data.index >= (now - timedelta(hours=period_hours))]
    
    # 데이터 확인용 출력
    print("USD Index Data")
    print(usd_index_data.head())
    print("USD/KRW Data")
    print(usd_krw_data.head())
    print("Last 24 Hours USD/KRW Data")
    print(last_24_hours_data.head())
    
    return usd_index_data, usd_krw_data, last_24_hours_data
...
def calculate_indicators(usd_index_data, usd_krw_data):
    ...
    today_usd_krw_data = usd_krw_data[usd_krw_data.index.strftime('%Y-%m-%d') == today]
    today_usd_krw_volume = today_usd_krw_data['Volume'].sum() if 'Volume' in usd_krw_data.columns else 'N/A'

    today_usd_index = round(float(usd_index_data['Close'].iloc[-1]), 2)  # 현재 달러 인덱스
    today_usd_krw = round(float(usd_krw_data['Close'].iloc[-1]), 2)  # 현재 USD/KRW 환율
    usd_index_median = round(float(usd_index_data['Close'].median()), 2)  # USD 인덱스 중앙값
    usd_krw_median = round(float(usd_krw_data['Close'].median()), 2)  # USD/KRW 환율 중앙값
    ...
    avg_usd_index = round(float(usd_index_data['Close'].mean()), 2)  # 평균 USD 인덱스
    avg_usd_krw = round(float(usd_krw_data['Close'].mean()), 2)  # 평균 USD/KRW 환율
    ...
    return {
        ...
        'today_usd_krw_volume': today_usd_krw_volume,
...
def create_result(period_weeks, indicators, conditions, advice, final_suitability, trend):
    ...
    return {
        ...
        '오늘 거래량': indicators['today_usd_krw_volume'],
...
def record_to_sheet(data):
    ...
    # 스프레드시트 열기 및 시트 선택
    sheet = client.open("적정환율알림서비스").worksheet("달러_4주")
    
    # 데이터 타입 변환 (JSON 직렬화 가능하도록)
    row = [data['현재 날짜'], data['기간']] + [str(value) if isinstance(value, (np.integer, np.floating)) else value for value in list(data.values())[2:]]
...
def calculate_trend(last_24_hours_data):
    ...
    :param last_24_hours_data: DataFrame, 최근 24시간 동안의 USD/KRW 환율 데이터
...
def calculate_exchange_rate(period_weeks, period_hours=24):
    ...
    """
    # 데이터 다운로드
    usd_index_data, usd_krw_data, last_24_hours_data = download_data(period_weeks, period_hours)

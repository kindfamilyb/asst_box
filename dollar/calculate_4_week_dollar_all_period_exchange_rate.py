    ...
    for date in usd_index_data.index:  # USD 인덱스 데이터의 각 날짜에 대해 반복
        if date in usd_krw_data.index:  # 해당 날짜가 USD/KRW 데이터에도 존재하는 경우
            ...
            if today_usd_index and today_usd_krw:  # 데이터가 존재하는 경우
                past_4_weeks = date - timedelta(weeks=4)  # 과거 4주 기간 설정
                
                # 과거 4주간의 중앙값과 평균 계산
                usd_index_median, avg_usd_index = calculate_median_and_mean(usd_index_data, 'Close', past_4_weeks, date)  # USD 인덱스 중앙값과 평균
                usd_krw_median, avg_usd_krw = calculate_median_and_mean(usd_krw_data, 'Close', past_4_weeks, date)  # USD/KRW 환율 중앙값과 평균
                
                # 갭 비율과 적정 환율 계산
                usd_gap_ratio_new = (today_usd_index / usd_krw_median) * 100  # 현재 USD 갭 비율
                avg_usd_gap_ratio = (usd_index_data.loc[past_4_weeks:date]['Close'] / usd_krw_data.loc[past_4_weeks:date]['Close']).mean() * 100  # 평균 USD 갭 비율
...
def record_to_sheet(data):  # 데이터를 스프레드시트에 기록하는 함수 정의
    ...
    # 스프레드시트 열기 및 시트 선택
    sheet = client.open("적정환율알림서비스").worksheet("4주기준달러전체기간적정환율예상수익")  # 스프레드시트와 시트 열기
